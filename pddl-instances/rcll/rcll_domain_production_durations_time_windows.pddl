...
(define (domain rcll-production-durative)
  ...
  ; to properly handle additional bases:
	...
	(:durative-action commit-to-ontime-delivery
		:parameters (?ord - order)
		:duration (= ?duration 0.01)
		:condition (and (at start (can-commit-for-ontime-delivery ?ord)))

		:effect (and (at end (not (can-commit-for-ontime-delivery ?ord))) (at end (committed-for-ontime-delivery ?ord)))
	)

	(:durative-action do-not-commit-to-ontime-delivery
		:parameters (?ord - order)
		:duration (= ?duration 0.01)
		:condition (and (at start (can-commit-for-ontime-delivery ?ord)))

		:effect (and (at end (not (can-commit-for-ontime-delivery ?ord))) (at end (not-committed-for-ontime-delivery ?ord)))
	)

...
configurations.append( ("icaps2018","./rewrite-no-lp --time-based-on-expansions-per-second 1000 --expansion-limit 200000  --forbid-self-overlapping-actions --deadline-aware-open-list Focal --slack-from-heuristic") )
				...
				for nexp in [1, 10, 100, 1000]:
					for real_time_multiplier in [1]:
						configurations.append( ("mt_improved_greedy__r_" + str(r) + "__gamma_" + str(gamma) + "__nexp_" + str(nexp) + "__tu_" + str(t_u) + "__min_pf_" + str(min_pf) + "__rtm_" + str(real_time_multiplier), 
							"./rewrite-no-lp --time-based-on-expansions-per-second 1000 --expansion-limit 200000 --include-metareasoning-time --real-to-plan-time-multiplier " + str(real_time_multiplier) + " --calculate-Q-interval " + str(r) + "  --min-probability-failure " + str(min_pf) + "  --slack-from-heuristic  --forbid-self-overlapping-actions --deadline-aware-open-list IJCAI --new-gamma " + str(gamma) + " --ijcai-t_u " + str(t_u) + " --icaps-for-n-expansions " + str(nexp) ) )
#						if gamma == 0:
...
for d in domains:
	for (i,(dom,prob)) in enumerate(domains[d]):
		...
		for cname,c in configurations:  
			print(c + " " + dom + " " + prob + " >& res/" + d + "/" + str(i) + "/" + cname + ".log")
